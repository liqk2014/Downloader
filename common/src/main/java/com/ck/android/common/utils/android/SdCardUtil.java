...
public class SdCardUtil {
    ...
    public static String getSDCardPath() {
        ...
        try {
            ...
            while ((lineStr = bufferedReader.readLine()) != null) {
                LogUtil.getInstance().i( "proc/mounts:   " + lineStr);
                if (lineStr.contains("sdcard")
                    && lineStr.contains(".android_secure")) {
                    String[] strArray = lineStr.split(" ");
                    if (strArray.length >= 5) {
                        sdcard = strArray[1].replace("/.android_secure", "");
                        LogUtil.getInstance().i("find sd card path:   " + sdcard);
                        return sdcard;
                    }
                }
                if (p.waitFor() != 0 && p.exitValue() == 1) {
                    // p.exitValue()==0表示正常结束，1：非正常结束
                    LogUtil.getInstance().e(cmd + " 命令执行失败");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        LogUtil.getInstance().i("not find sd card path return default:   " + sdcard);
        return sdcard;
    }
    ...
    public static ArrayList<String> getSDCardPathEx() {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                LogUtil.getInstance().i("mount:  " + line);
    ...
    public static SDCardInfo getSDCardInfo() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(state)) {
            ...
            LogUtil.getInstance().i( sd.toString());
